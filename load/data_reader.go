...
func (d *DataReader) Start(br *bufio.Reader, batchSize int, limit int64, reportPeriod time.Duration, metricCount, rowCount *uint64) int64 {
	d.start = time.Now()
	if reportPeriod.Nanoseconds() > 0 {
		go report(reportPeriod, metricCount, rowCount)
	}
	itemsRead := d.benchmark.Scan(batchSize, limit, br)
	close(d.inputDone)
