...
func TestGenericMetricsSimulatorRun(t *testing.T) {
	...
	hostPoints := make(map[string][]*serialize.Point, hostCount)
	...
	for !simulator.Finished() {
		...
		if write {
			...
			if _, ok := hostPoints[hostname]; !ok {
				hostPoints[hostname] = make([]*serialize.Point, 0)
			}
...
func assertHostPointDistribution(count int, host string, hostPoints map[string][]*serialize.Point, t *testing.T) {
	assertEqualInt(count, len(hostPoints[host]), fmt.Sprintf("Wrong host point distribution for host %s", host), t)
}

func assertMetricCountDistribution(count int, host string, hostPoints map[string][]*serialize.Point, t *testing.T) {
